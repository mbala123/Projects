...
namespace RegisterMvc.Controllers
{
    ...
    {
        static HttpClient svc = new HttpClient { BaseAddress = new Uri("http://localhost:5058/api/Register/") };
        ...
        [HttpPut]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit(Register r)
        {
            try
            {
                await svc.PutAsJsonAsync<Register>("",r);
                return RedirectToAction(nameof(Login));
            }
            catch
            {
